...
public class customer_script : MonoBehaviour
{
    ...
    private TemporaryTypeBonus TypeScript;
    ...
    {
        public List<string> type_1;
        public List<string> type_2;
        public List<string> type_3;
    }

    TypeWrapper AllTypes = new TypeWrapper();
    ...
    void Start()
    {
        ...
        TypeScript = TemporaryTypeBonus.reference;

        Button_options.InitGlobal();
        EfficiencyKey = upgrades.Dic_upgrades["Efficiency"];

        //Init data from a json file EXAMPLE 1
        /*
        //File:  {items: [ {Customer_types 1}, {Customer_types 2}]}
        items - List wrapper
        [System.Serializable]
        ...
        string type_data = JSON_operations.Read_file("All-types.json");
        TypeWrapper AllTypes = JSON_operations.SimpleFrom<TypeWrapper>(type_data);
        */

        string type_data = JSON_operations.Read_file("All-types.json");
        AllTypes = JSON_operations.SimpleFrom<TypeWrapper>(type_data);
    ...
    Dictionary<string, int> rngCustomer = new Dictionary<string, int>();
    int customerTotal = 0; //a sum of all weights
    ...
    void Log_client_gen(int id, Customer a)
    {
        ...
        + "; " + "AMOUNT=" + $"{a.buy_amount}" + "TYPES: " + $"{AllTypes.type_1[a.type1]}, " +  $"{AllTypes.type_2[a.type2]}, " + $"{AllTypes.type_3[a.type3]}; ");
    }
    ...
    public void Scene_init()
    {
        ...
        {
            person_list.Add(new Customer
            {
                x = Client_list[i].x,
                y = Client_list[i].y,
                type1 = Client_list[i].type1,
                type2 = Client_list[i].type2,
                type3 = Client_list[i].type3,
                item_id = Client_list[i].item_id,
                buy_amount = Client_list[i].buy_amount,
                score = Client_list[i].score,
                visits = Client_list[i].visits
            });
        }
    ...
    public void Generate_clients(int n)
    {
        ...
        for(int i = last_number; i < Client_number; i++)
        {
            ...
            int Tlen = AllTypes.type_1.Count;
            a.type1 = rand.Next(0, Tlen);
            Tlen = AllTypes.type_2.Count; 
            a.type2 = rand.Next(0, Tlen);
            Tlen = AllTypes.type_3.Count; 
    ...
    void weightCustomers()
    {
        ...
        {
            var empty = "";
            int weight = TypeScript.GiveTypeBonus(AllTypes.type_1[x.type1], AllTypes.type_2[x.type2], AllTypes.type_3[x.type3], empty);
            if (weight == 0)
                weight = 1;
            rngCustomer[i.ToString()] = weight;
    ...
    public void Start_buying()
    {
        ...
        //Type bonuses affect the chances of getting picked
        Chan.strPlace_weights(customerTotal, rngCustomer, out customerNumber);
        CNo = Convert.ToInt32(customerNumber);
        ...
        if (Stop == false)
        {
            ...
            //1 Modifies purchase amount by x
            int x_amount = TypeScript.GiveTypeBonus(AllTypes.type_1[person.type1], AllTypes.type_2[person.type2], AllTypes.type_3[person.type3], item_name);
            long product_amount = person.buy_amount + x_amount;
            GameLog.Message("Was buying: " + $"{person.buy_amount}, now: " + $"{product_amount}");
            string request = "Buying " + product_amount + "\n" + item_name;
            ...
            current_amount += x_amount;
            client_Panel.update_labels(person.score, AllTypes.type_1[person.type1], AllTypes.type_2[person.type2], AllTypes.type_3[person.type3], person.visits);
            //button_option handles the prices and other modifiers (types, score, etc.)
            //handles the button labels aswell

            Button_options.New_Customer(item_name, product_amount, person.score, current_amount, CNo, client_Panel, Cpanel);
        ...
        else if (Stop == true)
        {
            ...
            //Reset customer service amount
            Global_values.ServeCustomer = Global_values.WorkingCharges;
            //add a pop up
            //switch scene
        }
    }
    ...
    public void UseWorkCharge()
    {
        Global_values.ServeCustomer -= 1;
        Money_manager.update_turn_label((int)Global_values.Gamephase.Work);
        if (Global_values.ServeCustomer == 0)
        {
            Money_manager.update_turn_label((int)Global_values.Gamephase.Managment);
            Stop = true;
        }
    }

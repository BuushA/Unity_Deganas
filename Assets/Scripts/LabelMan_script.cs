...
public class LabelMan : MonoBehaviour
{
    ...
    public void update_turn_label(int Part)
    {
        string Turns = "";
        //Could be cached by principal but I'm lazy
        int current_turns = Global_values.turns % Global_values.TurnReset;
        Turns = "Turns: " + (current_turns + 1).ToString() + " / " + Global_values.TurnReset.ToString();

        if (Part == 1)
            Time_label_1.text = Turns;
        else if (Part == 2)
        {
            Time_label_2.text = Turns + "\n";
            Time_label_2.text += Global_values.ServeCustomer.ToString() + " / " + Global_values.WorkingCharges.ToString();
        }
